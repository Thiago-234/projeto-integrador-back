...
async function addMusica(event) {
    ...
    try {
        ...
        const response = await fetch('http://central-beat-adfsfearhfenb3dy.brazilsouth-01.azurewebsites.net/central', {
            method: 'POST',
            headers: {
...
async function deletarMusica() {
    try {
        ...
        const response = await fetch(`http://central-beat-adfsfearhfenb3dy.brazilsouth-01.azurewebsites.net/central/${musicaParaExcluir}`, {
            method: 'DELETE'
        });
...
async function infoMusica(id) {
    try {
        const response = await fetch(`http://central-beat-adfsfearhfenb3dy.brazilsouth-01.azurewebsites.net/central/${id}`); // URL para obter a música pelo ID
...
async function abrirEditar(idMusica) {
    ...
    try {
        const response = await fetch(`http://central-beat-adfsfearhfenb3dy.brazilsouth-01.azurewebsites.net/central/${musicaParaEditar}`);
        if (!response.ok) {
